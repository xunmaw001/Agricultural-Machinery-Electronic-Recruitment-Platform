<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="农机类型" prop="nongjileixing">
          <el-select :disabled="ro.nongjileixing" v-model="ruleForm.nongjileixing" placeholder="请选择农机类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="机主姓名" prop="jizhuxingming">
          <el-input v-model="ruleForm.jizhuxingming" 
              placeholder="机主姓名" clearable  :readonly="ro.jizhuxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="机主姓名" prop="jizhuxingming">
              <el-input v-model="ruleForm.jizhuxingming" 
                placeholder="机主姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	xiangqing : false,
      ...
      rules: {
          ...
          rizujia: [
                { required: true, message: '日租价不能为空', trigger: 'blur' },
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jizhuzhanghao = true;
          ...
          if(o=='jizhuxingming'){
            this.ruleForm.jizhuxingming = obj[o];
	    this.ro.jizhuxingming = true;
            continue;
          }
          if(o=='shoujihao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.jizhuzhanghao!=''&&json.jizhuzhanghao){
                this.ruleForm.jizhuzhanghao = json.jizhuzhanghao
	    		this.ro.jizhuzhanghao = true;
		}
		if(json.jizhuxingming!=''&&json.jizhuxingming){
                this.ruleForm.jizhuxingming = json.jizhuxingming
	    		this.ro.jizhuxingming = true;
		}
		if(json.shoujihao!=''&&json.shoujihao){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/nongjileixing/nongjileixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.nongjileixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springbootr910z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
