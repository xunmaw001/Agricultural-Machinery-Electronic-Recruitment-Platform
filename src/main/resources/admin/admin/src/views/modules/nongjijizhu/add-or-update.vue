...
export default {
  data() {
	...
	shenfenzheng : false,
      ...
      rules: {
          jizhuzhanghao: [
                { required: true, message: '机主账号不能为空', trigger: 'blur' },
          ],
          jizhuxingming: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `nongjijizhu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.nongjijizhuCrossAddOrUpdateFlag = false;
						 this.parent.search();
